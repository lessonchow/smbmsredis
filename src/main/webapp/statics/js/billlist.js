	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				//alert("对不起，订单【"+obj.attr("billcc")+"】不存在");
				changeDLGContent("对不起，删除订单【"+obj.attr("billcc")+"】操作异常");
			} 
		},
		error:function(data){
			alert("对不起，删除失败");
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/bill/getbillbyid?method=view&billid="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/bill/getbillbyid?method=modify&billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
